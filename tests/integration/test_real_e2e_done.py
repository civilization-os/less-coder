...
def test_real_e2e_chain_should_reach_done():
    async def run_case():
        root = Path(__file__).resolve().parents[2]
        fixture_src = root / "fixtures" / "java-sample"
        fixture_root = Path(tempfile.mkdtemp(prefix="java_fixture_done_"))
        ...
        # Ensure fixture starts from known buggy baseline.
